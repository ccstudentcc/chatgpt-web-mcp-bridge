...
describe('analyzeMcpTurn', () => {
  ...
  it('accepts rendered fallback cases when natural-language prefix appears before the fenced MCP block', async () => {
    ...
    expect(result.status).toBe('valid');
    expect(result.blocks).toHaveLength(1);
    expect(result.blocks[0]?.block.args.path).toBe('apps/extension/src/main/state.ts');
  });

  it('accepts a pure rendered MCP turn when the label line is separated from the json body by a newline', async () => {
    const visibleText = 'mcp\n{\n  "tool": "read_file",\n  "args": {\n    "path": "apps/extension/src/main/batch.ts"\n  }\n}';
    const fakeContainer = {
      querySelectorAll: () => [{
        textContent: 'mcp\n{\n  "tool": "read_file",\n  "args": {\n    "path": "apps/extension/src/main/batch.ts"\n  }\n}'
      }]
    ...
    expect(result.status).toBe('valid');
    expect(result.blocks).toHaveLength(1);
    expect(result.blocks[0]?.block.args.path).toBe('apps/extension/src/main/batch.ts');
  });
});
