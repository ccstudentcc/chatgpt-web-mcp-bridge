...
export interface StartExtensionRuntimeOptions {
  ...
  panelMountTarget?: HTMLElement | ShadowRoot | null;
}
...
export function startExtensionRuntimeWhenReady(options: StartExtensionRuntimeOptions = {}): void {
  ...
  const start = (): void => {
    void startExtensionRuntime(options);
  };
...
export async function startExtensionRuntime(options: StartExtensionRuntimeOptions = {}): Promise<void> {
  if (runtimePromise) {
...
async function deliverLastResult(readyStatus: ReadyDeliveryStatus): Promise<BridgeStatus> {
