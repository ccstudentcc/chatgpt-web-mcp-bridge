...
import { hasRg, runRg } from './rg-runtime.js';
import { createWorkspacePathPolicy, resolveBuiltinToolPath } from './workspace-policy.js';
import { matchesOptionalGlob, walkWorkspaceFiles, type WorkspaceFileEntry } from './workspace-search.js';
...
export const grepFilesTool: LocalTool<GrepFilesArgs, GrepFilesResult> = {
  ...
  async run(args, ctx) {
    ...
    let truncated = false;
    let totalMatches = 0;
    const candidateFiles = await listCandidateFiles(plan, args, policy);
...
): Promise<{ engine: GrepEngine; paths: WorkspaceFileEntry[] }> {
  if (plan.modeUsed === 'literal' && await hasRg()) {
    try {
      return {
        engine: 'rg',
        paths: await listCandidateFilesWithRg(plan, args, policy)
      };
    } catch {
      return {
        engine: 'node-fallback',
        paths: await listCandidateFilesWithNode(args, policy)
      };
    }
  }
...
async function listCandidateFilesWithRg(
  ...
  const stdout = await runRg(rgArgs, policy.workspaceRoot);
  const matches = new Map<string, WorkspaceFileEntry>();
  for (const line of stdout.split(/\r?\n/).map((value) => value.trim()).filter(Boolean)) {
    try {
      const safePath = await resolveBuiltinToolPath(line, policy);
...
function compareCandidateFiles(left: WorkspaceFileEntry, right: WorkspaceFileEntry): number {
