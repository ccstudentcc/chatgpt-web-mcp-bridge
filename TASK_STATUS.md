...
- Phase 2 has now advanced through the first gateway-side catalog, policy, and builtin implementation stages; the next open gateway extraction gate is `shell-runtime`.
...
- Stage 6 module ordering and rationale are now also explicit; Stages 7 `turn-runtime`, 8 `result-delivery`, 9 `injection-runtime`, 10 `operator-panel`, 11 `execution-kernel`, 12 `tool-registry`, 13 `tool-policy`, and 14 `builtin-tools` are complete, and the next open gate is now the active `shell-runtime` module stage.
...
- The current program gate is no longer v0.1 acceptance, and Phase 1 shared-contract freeze is no longer open. Stages 7 `turn-runtime`, 8 `result-delivery`, 9 `injection-runtime`, 10 `operator-panel`, 11 `execution-kernel`, 12 `tool-registry`, 13 `tool-policy`, and 14 `builtin-tools` are complete; the next active code-side extraction gate is now `shell-runtime`, without reopening closed browser-runtime modules or broad capability rollout.
...
- Primary axis: gateway shell process extraction after the completed registry, policy, and builtin implementation slices
...
- Still explicitly not opened while `shell-runtime` is the active module stage:
...
- `builtin-tools` is no longer the open gate; the next code-side gap is isolating shell detection, process guarding, timeout shaping, and captured shell output behind the `shell-runtime` owner boundary without turning shell execution into the default answer for structured repo actions.
