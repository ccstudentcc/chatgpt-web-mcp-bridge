...
- For non-trivial v0.9 work, read in this order:
  ...
  6. `TASK_STATUS.md`
- When working under `apps/extension` or `apps/gateway`, read the local `AGENTS.md` after this file and treat it as the subtree delta.
...
- When the user authorizes sustained v0.9 progress with automatic commits, work on the corresponding phase mainline branch by default, currently `agent/v0.9-phase2.5-mainline` for the latest Phase 2.5 line. Do not create per-stage branches unless the user explicitly asks for one, do not invent a new phase branch until the root task-control docs activate it, keep each commit scoped to one phase-aligned slice, and sync `TASK_STATUS.md` before every commit or session handoff.
...
- When a change modifies the gateway-to-extension runtime contract, such as `/health`, `/tools`, or `mcp_list` metadata, update the matching gateway/extension tests in the same pass.
...
- The canonical v0.9 code owner for ChatGPT Web page facts is `apps/extension/src/chatgpt-adapter/`. Do not create a second page-facts source of truth under archived legacy code.
...
- The v0.9 end state is `apps/extension` plus `apps/gateway`. `apps/userscript/` is an archived Stage 20 reference only; do not restore it as a live compat shell.
