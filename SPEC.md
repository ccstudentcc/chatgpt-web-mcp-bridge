...
- Whenever an assistant reply is issuing MCP tool calls, natural-language context may appear before the first fenced `mcp` block, but there must be no prose, raw JSON outside fences, or intermediate reasoning text between MCP blocks or after the first one appears.
